...
-   🔌 **LLM-Optimized Connection Management**
    ...
    -   **Outbound Connection & Request Optimization (for upstream LLM services):**
        ...
        -   Native support for LLM streaming responses (Server-Sent Events), ensuring efficient and complete data transmission.
...
| `upstream_groups[].http_client.timeout.request` | Integer | 300            | Request timeout (seconds), maximum waiting time from sending request to receiving complete response from upstream LLM (non-streaming). May need to be increased for long LLM requests |
