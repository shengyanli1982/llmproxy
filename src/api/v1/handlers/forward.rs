...
pub async fn list_forwards(State(config): State<Arc<Config>>) -> ApiResponse<Vec<ForwardConfig>> {
    let forwards = config.http_server.forwards.clone();
    info!("API: Retrieved {} forwarding rules", forwards.len());
    ApiResponse::success_with_data(forwards, "Successfully retrieved forwarding rules list")
}
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 404, description = "转发规则不存在 | Forwarding rule not found", body = ApiResponse<()>),
        (status = 500, description = "服务器内部错误 | Internal server error", body = ApiResponse<()>),
    ),
    security(
        ("bearer_auth" = [])
